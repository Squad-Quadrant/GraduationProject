...
namespace HighlightPlus2D {
	...
	public partial class HighlightEffect2D : MonoBehaviour {
		...
		Material GetMaterial(ref Material rmsMat, Material templateMat) {
			if (rmsMat == null || rmsMat == dummyMaterial || rmsMat.shader != templateMat.shader) {
		...
		void UpdateMaterialPropertiesNow() {
			...
			for (int k = 0; k < rmsCount; k++) {
				...
				if (shadowIntensity > 0) {
					...
					if (shadow3D && shadowDithering) {
						fxMat.EnableKeyword(ShaderParams.SHADOW_DITHERING_ON);
					} else {
						fxMat.DisableKeyword(ShaderParams.SHADOW_DITHERING_ON);
					}
