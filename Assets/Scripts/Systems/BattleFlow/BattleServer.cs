...
{
	public class BattleServer : IBattleServer, IDisposable
	{
			...
			IAIService aiService)
		{
            ...
            _eventBus.Subscribe<UnitDestroyedEvent>(CheckBattleOver);
		...
		public void Dispose()
		{
            ...
            _eventBus.Unsubscribe<UnitDestroyedEvent>(CheckBattleOver);
			IsRunning = false;
		}
        ...
        private void CheckBattleOver(UnitDestroyedEvent e)
        {
	        ...
	        _isGameOver = true;
	        _fsm.StopInteraction();

	        this.Log(isVictory
		        ? "All enemies defeated! You win!"
		        : "All player units defeated! You lose!");

	        _eventBus.Publish(new BattleOverEvent(isVictory));
        }
		...
		private void AwaitThen(Action trigger, Action onComplete, Action<AwaitPresentationCommand> configure)
		{
			...
			{
				var cmd = new AwaitPresentationCommand(GuardedComplete);
